    ...
    <div>
        ...
        <table class="table table-hover table-striped">
            ...
            <tr v-for="item in this.userlist" :key="item.id">
                ...
                <td class="py-1 icons">
                    <i title="دسترسی" class="fas fa-universal-access fa-2x m-1" @click="rolechange(item.id, item.name, item.roles)" v-if="accessshow"></i>
                    <i title="ویرایش" class="far fa-edit fa-2x m-1" data-toggle="modal" data-target="#Modal" @click="edituser(item.id, item.name, item.email)"></i>
                    <i title="حذف" class="fas fa-trash fa-2x m-1" @click="deleteuser(item.id)"></i>
                </td>
            </tr>
            </tbody>
        </table>
                            ...
                            <ul class="pl-0 pr-4">
                                <li class="list-group-item list-group-item-action pointer row" v-for="item in rolelist" :key="item.id" @click="changeroletick(item.order)">
                                    <div class="col-1 px-0 float-right">
                                        <i class="fas fa-check fa-lg" v-if="accessflag[item.order-1]"></i>
                                    </div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                accessshow: true,
                accessflag: {0 : false, 1 : false, 2 : false},
            }
        },
        methods: {
            saveuseraccess(){
